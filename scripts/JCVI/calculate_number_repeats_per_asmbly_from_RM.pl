...
while(<RM>){
	...
	# initialize %coverage
	$repeat_counter{ $line->{asmbl} }->{ $line->{repeat} } = 0 unless $repeat_counter{ $line->{asmbl} }->{ $line->{repeat} };
	
	
	$repeat_counter{ $line->{asmbl} }->{ $line->{repeat} } ++;
	$repeat_counter{ $line->{asmbl} }->{total} ++;
...
while(<ASMBL>){
	...
	if (exists $asmbl_list{$asmbl} ){
		...
		foreach my $repeat (keys %repeat_list){
			next unless $repeat;
			if (exists $repeat_counter{$asmbl}->{$repeat}){
				print "$asmbl\t$asmbl_len\t$repeat_counter{$asmbl}->{$repeat}\t$repeat\n";
			}
		}
		## print total coverage
		print "$asmbl\t$asmbl_len\t$repeat_counter{$asmbl}->{total}\ttotal\n";
	}
	else {
